...
var shoe = "http://www.zappos.com/nike-free-trainer-5-0-wolf-grey-pure-platinum-cool-grey-photo-blue?zfcTest=fw:1";
var shirt = "http://www.zappos.com/lacoste-classic-pique-polo-shirt-anise?zfcTest=fw:1";
var earings = "http://www.zappos.com/product/8016840/color/200873?zfcTest=fw:1";

var o = {};

r(shoe, function(err, resp, body) {
	var $ = c.load(body);
	...
	if ($('#productReviews').find('.additionalReviewsButton').length > 0) {
		...
		r('http://zappos.com'+href, function(err2, status2, body2) {
			...
			console.log(u.inspect(o,true,null));
		});
	}
});
...
function parseReviews($, list) {
	...
	$(list).each(function(i, review) {
		...
		if ($(this).find('.reviewRatings').length > 0) {
			$(this).find('.reviewRatings').children('span').each(function(x, s) {
				switch(x) {
					case 0: 
						name = 'overall';
						break;
		...
		if ($(this).find('.productFeel').length > 0) {
			rev.feel = {};
			rev.feel.shoeSize = $(this).find('.productShoeSize').children('p.feelIndicator').attr('class').match(/[0-9]/ig).toString();
			rev.feel.shoeWidth = $(this).find('.productShoeWidth').children('p.feelIndicator').attr('class').match(/[0-9]/ig).toString();
