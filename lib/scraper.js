...
function Scraper () {
	this._path = __dirname + '/scrapers';

	this._pool = require('./pool.js');
	
	this._TPL = this._path + '/template.js';
	this._MIN_WORKERS = 1;
	this._MAX_WORKERS = 5;
	this._KA_TIMER = 2000;
	this._EXPIRE = 5000;
	...
	this._MODULES = [
		{name: 'zappos.com', file_path: this._path + '/zappos.js'}
	];
...
*	remove - (pid), returns the pid of a child process which has been removed from the pool
*/
Scraper.prototype.init = function (opts) {
	...
	if (!this._isInitialized) {
		...
		this._pool.on('add', function(id) {
			console.log('POOL ADD: ', id);
			if (id instanceof Error) self.emit('error', id);
			else self.emit('add', id);
		})
...
Scraper.prototype.isReady = function() {
...
Scraper.prototype.getIdFromUrl = function(href) {
	var z = qs.parse(url.parse(href).query),
		u = url.parse(z.url),
		ret = {},
		x = u.pathname.split('/');
	switch (u.hostname) {
		case 'www.amazon.com':
			ret.name = qs.unescape(x[1]).replace(/[-_+.]/ig, ' ');
			ret.id = x[3];
			break;
		case 'www.zappos.com':
			if (x[1] === 'product') {
