...
exports.db = {
	...
	remove: function(params, cb) {
		if ('id' in params) {
			params.rev ? this._db.remove(params.id, params.rev, back) : this._db.remove(params.id, back);
		} else return cb(new Error('first argument must contain a document id'));
	...
	view: function(params, cb) {
		if ('view' in params) {
			params.key ? this._db.view(params.view, {key: params.key}, back) : this._db.view(params.view, back);
