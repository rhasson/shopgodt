				...
				cache.hget('pins', req.session.fb.fb_id, function(err2, item) {
					if (!err2) {
						...
						r.post({url: url}, function(err, resp, b) {
							...
							if (!err && !b.error && resp.statusCode === 200) {
								req.session.fb.wall_post = d.id;
								next();
							} else if (b.error) {
								if (b.error.type === 'OAuthException' && b.error.code === 190 && b.error.error_subcode === 467) {
									req.session.destroy(function(err) {
										if (err) console.log('failed to destroy session', err);
										else {
