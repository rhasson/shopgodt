...
Facebook.prototype.redirect = function(o) {
	...
	return function redirect (req, res, next) {
		...
		if ('error' in req.query) {
			...
			req.session.fb.error = self._error;
			return next();
		} else {
			...
			r(url, function(err, resp, body) {
				if (!err && resp.statusCode == 200) {
					...
					req.session.fb.instance.push(self);
					return next();
...
Facebook.prototype.isAuthenticated = function(req) {
	if (req.session.fb) {
		if (!req.session.fb.error && req.session.fb.access_token) {
			x = ((Date.now() - req.session.fb.expires) < 0) ? true : false;
			return x;
		} else return false;
	}
}
