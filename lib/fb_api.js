...
Facebook.prototype.logout = function(o) {
	...
	return function logout(req, res, next) {
		...
		if (req.session.fb) {
			...
			cache.hdel('friends', req.session.fb.fb_id);
						...
						r.post({url: url}, function(err, resp, b) {
							if (!err && resp.statusCode === 200) {
								...
								try {
									...
									d = JSON.parse(b);
									var u = self.graph+d.id+qs.stringify({access_token: req.session.fb.access_token});
									r.get({url: u}, function(e2,r2, b2){
										console.log('POST', e2, b2);
									});
