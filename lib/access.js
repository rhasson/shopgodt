			...
			this.view({
				...
				if (err) return cb(err);
					...
					self.save({body: params}, function(err2, newDoc) {
						...
						else if (newDoc.ok) return cb(null, {id: newDoc.id, fb_id: params.fb_id});//newDoc.id+':'+params.fb_id);
					});
				}
				else if (doc.length > 0) {
					self.update({id: doc[0].id, body: params}, function(err3, upDoc) {
						if (err3) return done(err3);
						else if (upDoc.ok) return cb(null, {id: upDoc.id, fb_id: params.fb_id});//upDoc.id+':'+params.fb_id);
					});
				} else return cb(new Error('Error with log in and registration process'));
			});
			break;
		default:
			self.save({body: params}, function(err, doc) {
				if (err) return cb(err);
...
Access.prototype.isReady = function() {
	return this.isReady;
};
...
var _set = function(params, cb) {
	db.save(params, function(err, data) {
		if (!err) {
			cb(null, data);
		} else {
